...
- **LLM 闸门第一版占位**:`core/llm_explainer.py` 第一版返回固定字符串(沿用启发式占位),但接口预留,接 LLM 时只改这个文件,不动其它。
...
### 9.1 仓位与日亏
- **RiskAmount 基线** = $50 / 单(FTMO 一阶段日亏上限 $300 = 一天能亏 6 次,本基线据此定)
- **按类目系数**(防加密/小盘股爆仓):
  - 外汇:×1.0 → $50/单
  - 指数:×1.0 → $50/单
  - 商品:×0.7 → $35/单
...
| 8 | LLM 闸门 | **第一版占位** ✅ | 第一版就接 |
| 9 | DEMO 模式 | 保留 ✅ | 不要 |
| 10 | 凭据流 | "下单专用 init"独立链路 ✅ | 主进程共享 |

> 全部默认项已锁,后续修改请直接 patch 此表。

---

## 16. 给协作者的回信(开工前回这 3 个)
