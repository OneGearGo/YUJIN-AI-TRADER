...
def api_run(r: RunIn):
    ...
    # MT5 thin-proxy: chain=mt5* routes to aitrader_adapter.build_scanner_payload().
    # MT5 thin-proxy: force mt5 regardless of frontend default of sol
    ch_requested = "mt5"
    if ch_requested in ("mt5", "mt5fx", "fx", "mt", ""):
        try:
            from aitrader_adapter import build_scanner_payload
            payload = build_scanner_payload(chain=ch_requested)
            if isinstance(payload, dict):
                payload["mode"] = "LIVE"
                payload["chain"] = "mt5"

                # contract aliases so frontend normalize() (which reads data.list / data.risk) finds them
                if "decisions" in payload and "list" not in payload:
                    payload["list"] = payload["decisions"]
                if "portfolio" in payload and "risk" not in payload:
                    payload["risk"] = payload["portfolio"]
            return JSONResponse(payload)
        except Exception as e:
            try:
                with ST.lock:
                    return JSONResponse(screen_once("sol"))
            except Exception as e2:
                raise HTTPException(502, f"MT5 scan failed: {e}; mock fallback also failed: {e2}")
...
def api_positions(chain: str = "sol"):
    # MT5 thin-proxy: real positions live in F:/yujin-mt5/app.py; here return empty to
    # avoid aitrader monitor_positions walking solana path.
    ...
    if PUBLIC_DEMO:
        return dict(positions=[], portfolio=empty_portfolio)
    if (chain or "mt5").lower() in ("mt5", "mt5fx", "fx", "mt"):
        return dict(positions=[], portfolio=empty_portfolio)
    ch = valid_chain(chain)
    with ST.lock:
